...
var getProjectByDirectory = exports.getProjectByDirectory =
  ...
  if (results.length === 1) {
    ...
    throw new Error(util.format('More than one result returned for "%s": %j',
      directory, results));
  }
};
...
var getProjectOrDie = exports.getProjectOrDie = function (name) {
  var project;
...
var minimatchByName = exports.minimatchByName = function (name) {
  return all().filter(function (project) {
    return minimatch(project.name, name);
  });
};

exports.getProjectOrProjectsOrDie = function (nameOrNames) {
  if (!Array.isArray(nameOrNames)) {
    nameOrNames = [nameOrNames];
  }

  var results = _(nameOrNames)
    .map(function (name) {
      if (name.indexOf('*') !== -1) {
        return minimatchByName(name);
      }

      return getProjectOrDie(name);
    })
    .flatten()
    .uniq('name')
    .valueOf();
  ...
  if (!results) {
    console.error('Could not find any matching projects.');
